...
Entry-level cybersecurity and technical support professional with hands-on experience in Linux systems, Python development, monitoring, and security operations fundamentals.
...
## 🏆 Certifications
- **CompTIA Security+** – Issued May 2026 | [Verify on Credly](https://credly.com/badges/57a14b65-7cef-4d5b-881b-b6fd7280ae21/public_url)
...
- Implemented controls for creating, editing, and applying remapping rules.  
- Focused on improving usability for Linux input customization.  
- Integrated external Linux tooling into a desktop application.  

**Technologies:** Python, Tkinter, Linux, xremap

---

### 🔹 [ACTrigger](https://github.com/bytesentry/ACTrigger)

C#/.NET application and Decal plugin providing real-time combat tracking, configurable triggers, debuff tracking, scrolling combat text, and customizable in-game HUD elements for Asheron's Call.

- Developed a Decal plugin that subscribes directly to the game's chat/event stream and processes combat events within the plugin.
- Implemented parsing and classification of incoming and outgoing damage events for real-time combat tracking.
- Developed an event-driven pipeline between the Decal plugin and companion desktop application using a generated event log.
- Built transparent overlay windows for scrolling incoming and outgoing combat text.
- Implemented a debuff tracker that identifies effects and maintains active debuff timers.
- Implemented configurable text triggers that respond to detected events with associated audio notifications.
- Developed a custom HUD/nameplate rendering system that generates name, level, and monarch elements independently before composing them into final in-game assets.
- Implemented entity nameplate management for visible game objects through the Decal plugin.
- Built the application using asynchronous processing, state management, caching, and multi-component coordination to support continuous real-time operation.

**Technologies:** C#, .NET, Avalonia UI, Decal, MVVM, event-driven programming, asynchronous processing, real-time event processing, HUD rendering
...
- Implemented visual progress tracking and sound notifications.  
- Designed minimal interface focused on usability and low resource usage.  
- Implemented timing and event-driven UI logic.  
...
- Implemented alerting and notification logic.  
- Designed architecture applicable to SOC/NOC-style monitoring scenarios.  
- Demonstrates event-driven programming and real-time processing.  
...
### 🔹 Marketplace Data Collection & Search Platform
Application that collects marketplace listing data from a third-party web platform and stores it in a searchable SQL-backed database.

- Implemented web scraping logic using HTTP requests and session handling.  
- Parsed HTML responses to extract structured item data.  
- Designed relational SQL schema for items and listings.  
- Implemented search and filtering queries.  
- Built tooling to retrieve detailed item metadata on demand.  
- Focused on responsible request behavior and rate limiting.  

**Technologies:** Python, Requests, SQL  
...
## 🛠 Systems & Hardware Experience

- Assembled and configured custom desktop systems.  
- Installed and configured Windows, Linux, and macOS operating systems.  
